...
export type BackendEvents = DefineEvents<{
  "bytecap:threshold-alert": (alert: { type: "error" | "warning", message: string }) => void;
  "bytecap:file-scan-complete": (summary: { fileCount: number, caidoFileCount: number, totalSize: string, caidoTotalSize: string }) => void;
}>;
...
const getWorkspaceFiles = async (sdk: BytecapBackendSDK, workspacePath?: string): Promise<DirectoryInfo> => {
  ...
  try {
    ...
    if (workspacePath) {
      ...
      sdk.console.log(`Caido projects dir: ${caidoProjectsDir} exists: ${pathExists(projectDir)}`);
    ...
    const caidoTotalSize = caidoFiles.reduce((total, file) => total + file.size, 0);
    
    sdk.console.log(`Files processed: ${files.length} Total size: ${formatBytes(totalSize)}`);
    sdk.console.log(`Caido files found: ${caidoFiles.length} Combined .caido size: ${formatBytes(caidoTotalSize)}`);
...
const checkFileSizeThresholds = (sdk: BytecapBackendSDK, directoryInfo: DirectoryInfo, thresholdMB: number, enableWarnings: boolean, warningPercentages: number[] = [75, 90]): { alerts: string[], warnings: string[] } => {
  ...
  if (directoryInfo.caidoFiles.length > 0) {
    if (directoryInfo.caidoTotalSize >= thresholdBytes) {
      ...
      sdk.api.send("bytecap:threshold-alert", { type: "error", message: alertMessage });
      ...
      for (const percentage of warningPercentages) {
        ...
        if (directoryInfo.caidoTotalSize >= warningThreshold) {
          ...
          // Send threshold alert event
          sdk.api.send("bytecap:threshold-alert", { type: "warning", message: warningMessage });
          break;
        }
      }
    }
